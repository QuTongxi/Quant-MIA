...
def select_and_set_device(dev_id = -1):
    if dev_id != -1:
        ...
        dev = select_best_gpu()
        if dev != 'cpu':
            os.environ["CUDA_VISIBLE_DEVICES"] = dev.split(':')[-1]
...
def load_one(save_dir, dataset, datapath):
    """
    This loads a logits and converts it to a scored prediction.
    """
    opredictions = np.load(os.path.join(save_dir, "logits.npy"))  # [n_examples, n_augs, n_classes]
    ...
    labels = get_labels(dataset, datapath)  # TODO generalize this
    ...
    np.save(os.path.join(save_dir, "scores.npy"), logit)

def get_labels(dataname, dpath):
    if dataname == 'cifar10':
        dataset = CIFAR10(root=dpath, train=True, download=False)
        return np.array(dataset.targets)
    elif dataname == 'cifar100':
        dataset = CIFAR100(root=dpath, train=True, download=False)
        return np.array(dataset.targets)
    elif dataname == 'tiny-imagenet':
        dataset = TinyImageNet(dpath, train=True, transform=None)
        all_labels = []
...
def run_inference(queries, train_dl, model, save_dir):
    ...
    path = os.path.join(save_dir, 'logits.npy')
